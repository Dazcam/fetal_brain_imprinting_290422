...
for GENE in GRB10 PEG3; do

#`awk -F"\t" '$4>0' ase_imprinting_genes_summary | awk '{print $1}'`; do
  ...
  SAMPLES=`grep -c txt snps/${GENE}/${SNP}_ase`
  ...
  if ($4 >= imp_upper || $4 <= imp_lower)
  	  print $1"\t"$4"\tImprinted" > "snps/"gene"/"snp"_imp_test"; 
  else if ($4 >= part_upper || $4 <= part_lower)
      print $1"\t"$4"\tPartially imprinted" > "snps/"gene"/"snp"_imp_test";
  else
	  print $1"\t"$4"\tBiallelic" > "snps/"gene"/"snp"_imp_test";
  }' 
  ...
  IMPRINTED=`grep -c Imprinted snps/${GENE}/${SNP}_imp_test`
  PROP_IMPRINTED=$(echo "${IMPRINTED}/${SAMPLES}" | bc -l) # Feed calc to bc as floating pt

  PART_IMP=`grep -c Partially snps/${GENE}/${SNP}_imp_test`
