...
## Set variables ----------------------------------------------------------------------
for GENE in `awk -F"\t" '$4>0' ${INDIR}summary_6_tucci_ase_imprinting_genes_summary.txt | awk '{print $1}'`; do
  ...
  if ($4 >= imp_upper || $4 <= imp_lower)
          print $1"\t"$4"\tImprinted" > "../results/16ISGENEIMPRINTED_TUCCI/"gene"/"snp"_imp_test";
  else if ($4 >= part_upper || $4 <= part_lower)
      print $1"\t"$4"\tPartially imprinted" > "../results/16ISGENEIMPRINTED_TUCCI/"gene"/"snp"_imp_test";
  else
          print $1"\t"$4"\tBiallelic" > "../results/16ISGENEIMPRINTED_TUCCI/"gene"/"snp"_imp_test";
