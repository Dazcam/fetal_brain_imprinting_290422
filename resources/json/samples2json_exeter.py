...
for root, dirs, files in os.walk(args.fastq_dir):
        for file in files:
                if file.endswith("fastq"):
                        ...
                        #Filename structure: 15533_TGACCA_L007_R1_001.fastq
                        m = re.search(r"([0-9]{5})_([A-Z]{6})_(L[0-9]{3})_(R[12])_[0-9]{3}.fastq", file)
                        if m:
                             	sample = m.group(1)
                                adapter = m.group(2)
                                lane = m.group(3)
...
open('samples_exeter.json', 'w').writelines(js)
